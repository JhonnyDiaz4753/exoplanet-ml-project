...
class ResNetLSTM1D(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)
        x = self.layer2(x)
        x = self.layer3(x)  # Output shape: [B, C=64, T]
        x = x.permute(0, 2, 1)  # Reshape to [B, T, C] for LSTM
        x, _ = self.lstm(x)
...
# Puedes usar esta clase como reemplazo directo en tu pipeline:
# model = ResNetLSTM1D().to(device)
# y seguir con tu función train_model y evaluate_model existentes
