...
def visualize_light_curves(X, y, output_dir='reports/figures/CurvasEjemplo/', n=5):
    os.makedirs(output_dir, exist_ok=True)
    X = np.array(X)
    y = np.array(y)

    pos_idx = np.where(y == 1)[0][:n]
    neg_idx = np.where(y == 0)[0][:n]

    for i, idx in enumerate(pos_idx):
        plt.plot(X[idx], color='blue')
        plt.title(f"Curva de Luz Positiva #{i+1}")
        plt.xlabel("Tiempo")
        plt.ylabel("Intensidad normalizada")
        plt.savefig(f"{output_dir}/curva_pos_{i+1}.png")
        plt.close()

    for i, idx in enumerate(neg_idx):
        plt.plot(X[idx], color='red')
        plt.title(f"Curva de Luz Negativa #{i+1}")
        plt.xlabel("Tiempo")
        plt.ylabel("Intensidad normalizada")
        plt.savefig(f"{output_dir}/curva_neg_{i+1}.png")
        plt.close()

    print(f"Curvas de ejemplo guardadas en {output_dir}")
