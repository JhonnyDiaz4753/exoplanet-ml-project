...
class ResNetLSTM1D(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)
        x = self.layer2(x)
        x = self.layer3(x)  # [B, 64, T]
        x = x.permute(0, 2, 1)  # [B, T, 64]
        x, _ = self.lstm(x)
        x = x[:, -1, :]  # Última salida temporal
        x = self.dropout(x)
        x = self.fc(x)
        return torch.sigmoid(x)

# === ATENCIÓN SIMPLE PARA LSTM ==============================
class Attention(nn.Module):
    ...
    def forward(self, lstm_out):
        weights = self.attn(lstm_out).squeeze(-1)  # [B, T]
        weights = torch.softmax(weights, dim=1)
        context = torch.sum(lstm_out * weights.unsqueeze(-1), dim=1)  # [B, 2H]
        return context

# === MODELO 2: CNN + LSTM + ATENCIÓN ========================
...
class ResNetLSTM1DWithSEAttention(nn.Module):
    def __init__(self, lstm_hidden=64, lstm_layers=1):
        super().__init__()
        ...
        self.layer1 = BasicBlock1D(1, 16)
        self.layer2 = BasicBlock1D(16, 32, downsample=True)
        self.se1 = SEBlock1D(32)  # SE block after conv2

        self.layer3 = BasicBlock1D(32, 64, downsample=True)
        self.se2 = SEBlock1D(64)  # SE block after conv3
        ...
        # Attention layer replaced by SE block (or keep Attention if preferred)
    ...
    def attention(self, lstm_output):
        # lstm_output: [B, T, 2H]
        weights = self.se_attention(lstm_output)  # [B, T, 1]
        weights = torch.softmax(weights, dim=1)   # [B, T, 1]
        attended = torch.sum(lstm_output * weights, dim=1)  # [B, 2H]
        return attended
    ...
    def forward(self, x):
        ...
        x = self.attention(lstm_out)  # [B, 2H]
        x = self.dropout(x)
        x = self.fc(x)
        return torch.sigmoid(x)

# === MODELO 3: TRANSFORMER PURAMENTE ========================
class Transformer1D(nn.Module):
    def __init__(self, d_model=64, nhead=4, num_layers=2, dim_feedforward=128, dropout=0.1):
        super().__init__()
        self.input_proj = nn.Linear(200, d_model)  # [B, 200] → [B, d_model]
...
def train_model(X, y, output_path='models/cnn_model.pt', epochs=100, batch_size=16, lr=0.0005, patience=10, model_name='cnn_lstm_attn'):
    ...
    if model_name == 'cnn_lstm':
     ...
     model = ResNetLSTM1DWithSEAttention().to(device)
     print("Training Improved CNN + LSTM + ATENCIÓN model...")
